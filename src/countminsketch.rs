...
use num_traits::{CheckedAdd, One, Saturating, Unsigned, Zero};
...
where
    T: Hash,
    C: Saturating + CheckedAdd + Clone + One + Ord + Unsigned + Zero,
...
where
    T: Hash,
    C: CheckedAdd + Saturating + Clone + One + Ord + Unsigned + Zero,
    B: BuildHasher + Clone + Eq,
{
    ...
    pub fn merge(&mut self, other: &Self) {
        ...
        self.table = self
            ...
            .map(|x| x.0.clone().saturating_add(x.1.clone()))
            .collect();
    }
